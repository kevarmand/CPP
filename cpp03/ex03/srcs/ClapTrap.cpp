...
/*   Updated: 2025/08/23 19:22:27 by kearmand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::attack(const std::string& target) {
	std::cout << "\U00002694  ";
	if (_energyPoints <= 0) {
		std::cout << ORANGE "[WARNING] : " RESET "ClapTrap " BOLD_BLUE_SOFT << _name << RESET " has no energy points left to attack." << std::endl;
		return;
	}
	if (_hitPoints <= 0) {
		std::cout << ORANGE "[WARNING] : " RESET "ClapTrap " BOLD_BLUE_SOFT << _name << RESET " has no hit points left and cannot attack." << std::endl;
		return;
	}
	_energyPoints--;
